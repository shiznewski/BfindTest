...
function BarInfo(type){
	...
	//alert('hi');
	var username = "shiznewski";
	var password = "barpass";
	
	var str = "type=" + type;
	var str = str + "&username=" + username + "&password=" + password;
	if (type == 'update'){
		str = $("#inventory").serialize() + '&' + str;
	}
	if ((type != 'update') && (type != 'load')){
		var mysplit = type.split(",");
		str = "type=delete&barid=" + mysplit[0] + "&beerid=" + mysplit[1] + "&type=" + mysplit[2] + "&username=" + username + "&password=" + password;
	}
	//alert(str);
	$.ajax({
		type: "POST",
